...
import type { FileSystemSyncAccessHandle } from "../src/file_system_sync_access_handle.ts";
import type { Provider } from "@test/util.ts";
...
export function runFileSystemSyncAccessHandleTest(provider: Provider): void {
  describe("FileSystemSyncAccessHandle", () => {
    beforeEach<Context>(async function () {
      const context = await provider();
      const handle = await context.root.getFileHandle("file.txt", {
        create: true,
      });
